...
class Gmailfs(Fuse):
	...
	#@+node:__init__
	def __init__(self, extraOpts, mountpoint, *args, **kwargs):
		"""
...
#04:52.97 < * CAPABILITY IMAP4rev1 UNSELECT LITERAL+ IDLE NAMESPACE QUOTA ID XLIST CHILDREN X-GM-EXT-1 UIDPLUS COMPRESS=DEFLATE
		...
		if trash_all:
			...
			resp, msgids = imap_uid(self.imap, "SEARCH", 'ALL')
	...
	def getattr(self, path):
		...
		if not inode and path == '/': #(inode == None) and (path == '/'):
	...
	def readdir(self, path, offset):
		...
		for r in lst:
			...
			yield fuse.Direntry(r) # NOTE: ah generator, this is unportable to rpython/pypy
	#@-node:getdir
	...
	def flush_dirent_cache(self):
		...
		while 1: # WARNING: WTF, BUSY WAIT?!
			try:
				...
				break
#		size = self.fs.dirty_objects.qsize() # WARNING: neither the parent class or current class has a field called 'fs'
		size = self.dirty_objects.qsize()
	...
	def read(self, path, readlen, offset):
		log_entry("read")
		try:
			log_debug1("gmailfs.py:Gmailfs:read(len=%d, offset=%d, path='%s')" % (readlen, offset, path))
